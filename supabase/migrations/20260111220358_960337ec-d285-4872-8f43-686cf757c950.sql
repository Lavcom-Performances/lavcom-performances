...
-- D'abord supprimer les fonctions avec des signatures différentes
DROP FUNCTION IF EXISTS public.rpc_dashboard_kpis(uuid, date, date);

-- Recréer la fonction dashboard KPIs pour exclure les rechargements
...
BEGIN
  ...
  SELECT 
    ...
    (
      SELECT EXTRACT(HOUR FROM (o2.operation_time || ':00')::time)::integer
      FROM operations o2
      ...
      GROUP BY EXTRACT(HOUR FROM (o2.operation_time || ':00')::time)
      ORDER BY COUNT(*) DESC
      LIMIT 1
    ) as peak_hour
  FROM operations o
...
-- Grant permission
GRANT EXECUTE ON FUNCTION public.rpc_dashboard_kpis(uuid, date, date) TO authenticated;
