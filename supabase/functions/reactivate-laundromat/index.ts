...
interface ReactivateLaundromatRequest {
...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    // Parse request body
    const body: ReactivateLaundromatRequest = await req.json();
    const { laundromat_id } = body;
    ...
    if (!hasPermission) {
      return new Response(
        JSON.stringify({ error: 'Only owners and admins can reactivate a laundromat' }),
    ...
    if (site.status === 'active') {
      return new Response(
        JSON.stringify({ success: true, message: 'Laundromat is already active' }),
    ...
    if (updateError) {
      console.error('Error reactivating laundromat:', updateError);
      return new Response(
        JSON.stringify({ error: 'Failed to reactivate laundromat' }),
    ...
    await serviceClient.from('system_events').insert({
      source: 'reactivate-laundromat',
      severity: 'info',
      code: 'laundromat_reactivated',
      message: `Laundromat "${site.name}" was reactivated`,
      meta: {
        laundromat_id,
    ...
    console.log(`Laundromat ${laundromat_id} reactivated by user ${userId}`);
    ...
    console.error('Error in reactivate-laundromat:', error);
    return new Response(
